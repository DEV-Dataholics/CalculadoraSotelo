...
function BoletaDetail({ trip, onUpdate, dieselPrice, unitYields, defaultYield }) {
  ...
  const calculateDependentFields = (rowData, allRows) => {
    // Fix 4: usar precio individual del row (Peso_Carga) si fue capturado, o precio global dp
    const currentDieselPrice = parseFloat(rowData.Peso_Carga) > 0 ? parseFloat(rowData.Peso_Carga) : dp
    ...
    // Fix 1: dieselAFavor usa precio individual del row para visualización en tiempo real
  ...
  const recalcAndNotify = (currentRowsData) => {
    ...
    // Fix 1: sumar diésel a favor por row individualmente (precio individual por row)
    const totalDieselAFavor = currentRowsData.reduce((sum, r) => {
        const precioPorRow = parseFloat(r.Peso_Carga) > 0 ? parseFloat(r.Peso_Carga) : dp
        const litrosRow = parseFloat(r.Litros_A_Pago) || 0
        return sum + (litrosRow * precioPorRow)
    }, 0)

    // Fix 1: Total = base + cruce + quimico + bonos + diesel_a_favor
    const incentivePay = quimico + bonos + pagoCruceVal + totalDieselAFavor
    const updatedRows = (trip.Rows || []).map((or, i) => ({ ...or, ...currentRowsData[i] }))

    // ADR-003: Incentive_Pay para visualización en tiempo real.
    // Total_Pay definitivo llega en el response de /api/calculate.
    onUpdate({
        ...trip,
        Rows: updatedRows,
  ...
  // Fix 4: totalDiésel usa precio individual por row (Peso_Carga) si disponible, o precio global dp
  const totalDiésel = rowsData.reduce((sum, r) => {
    const precioPorRow = parseFloat(r.Peso_Carga) > 0 ? parseFloat(r.Peso_Carga) : dp
    const litrosRow = parseFloat(r.Litros_A_Pago) || 0
    return sum + (litrosRow * precioPorRow)
  }, 0)
  ...
  if (trip.Is_Pacifico) {
      ...
      if (bonoDoble) pacificoBonosVal += 2439  // Fix 5: valor correcto validado en nóminas históricas
